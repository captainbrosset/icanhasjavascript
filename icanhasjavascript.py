...
def get_file_content_entities(file_content):
...
def get_model(source_directories, callback):
    ...
    for source_dir in source_directories:
        ...
        print "[ICANHASJAVASCRIPT] Scanning " + str(len(files)) + " javascript files in " + source_dir + "..."
        for file in files:
            content = get_file_content(file)
            entities = get_file_content_entities(content)
...
class ICanHasJavaScript(sublime_plugin.EventListener):
    ...
    def construct_model(self, view):
        if not self._is_constructing_model:
            print "[ICANHASJAVASCRIPT] Building the model now ..."
    ...
    def on_model_constructed(self, model):
        print "[ICANHASJAVASCRIPT] Done building model"
        self.model = model
        self._is_constructing_model = False
...
if __name__ == "__main__":
    ...
    assert len(get_file_content_entities(content)) == 3

    files = get_all_js_files_in_dir("testdir")
    assert len(files) == 24
    assert files[0] == "testdir/apple.js"
    assert files[23] == "testdir/visibility.js"
    ...
    def assert_model_retrieved(model):
        ...
        assert get_suggestions("tes", model)[1].file_name == "testdir/syntaxerror.js"
